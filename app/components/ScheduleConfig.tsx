...
export default function ScheduleConfigComponent({ value, onChange }: ScheduleConfigProps) {
  ...
  const [enableScanLimit, setEnableScanLimit] = useState(value.scanLimit !== null && value.scanLimit !== undefined)
  ...
  const handleScanLimitChange = (limit: string) => {
    const numLimit = limit === '' ? null : parseInt(limit, 10)
    onChange({
      ...value,
      scanLimit: enableScanLimit ? (isNaN(numLimit as number) ? null : numLimit) : null,
    })
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-neutral-500 mt-1">
              Create time ranges for your business and apply them to your QR code
            </p>
          </div>
      ...
      <div className="border border-neutral-200 rounded-lg p-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-neutral-900">Schedule a Date</h3>
            <p className="text-xs text-neutral-500 mt-1">
              Set the dates on which your content will be shown. At the end of that period, it will be disabled.
            </p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
      ...
      <div className="border border-neutral-200 rounded-lg p-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-neutral-900">Scan Limit</h3>
            <p className="text-xs text-neutral-500 mt-1">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) => {
                setEnableScanLimit(e.target.checked)
                if (!e.target.checked) {
                  onChange({ ...value, scanLimit: null })
                }
              }}
              className="sr-only peer"
            />
        ...
        {enableScanLimit && (
          <div className="mt-4">
            <label className="block text-xs font-medium text-neutral-700 mb-1">Maximum Scans</label>
            <input
              type="number"
              min="1"
              value={value.scanLimit !== null && value.scanLimit !== undefined ? value.scanLimit : ''}
              onChange={(e) => handleScanLimitChange(e.target.value)}
              placeholder="e.g. 100"
              className="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500"
            />
            <p className="mt-1 text-xs text-neutral-500">
              Once this limit is reached, the QR code will no longer be accessible
            </p>
          </div>
        )}
      </div>
